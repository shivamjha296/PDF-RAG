...
@app.get("/")
async def root():
    return {"message": "Go to /app to use the interface"}
...
async def chat_endpoint(
    ...
    """
    Handle chat with PDF (URL or Upload).
    """
    # 1. Get PDF Content
    chunks = []
    pdf_hash = ""
    ...
    if file:
        ...
        pdf_hash = get_pdf_hash(content)
        
        # Check cache
        cached_chunks, _, _ = await load_embeddings_from_cache(pdf_hash)
        if cached_chunks:
             ...
             chunks_data = advanced_chunking(text)
             chunks = [c["text"] for c in chunks_data]
             
             # Generate embeddings
             embeddings = await generate_embeddings_parallel(chunks)
             await save_embeddings_to_cache(pdf_hash, chunks, embeddings, pdf_url="upload", chunks_data=chunks_data)
             
    elif url:
        # Use existing logic
        chunks, pdf_hash, _, _ = await download_and_parse_pdf_async(url)
        
        cached_chunks, cached_embeddings, _ = await load_embeddings_from_cache(pdf_hash)
        if cached_embeddings is None:
             embeddings = await generate_embeddings_parallel(chunks)
             await save_embeddings_to_cache(pdf_hash, chunks, embeddings, pdf_url=url)
    ...
    # 2. Answer Question
    # Load embeddings
    _, embeddings, _ = await load_embeddings_from_cache(pdf_hash)
    
    # Embed question
    question_embedding = await generate_embeddings_nomic_async([question])
